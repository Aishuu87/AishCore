-- AishCore.lua : Point d'entree principal (init + event dispatch)
local addonName, ns = ...

-- Frame principal pour les evenements
local frame = CreateFrame("Frame", "AishCoreMainFrame", UIParent)
frame:SetSize(1, 1)
...
local function DPrint(msg)
  if ns._diag then
    print("|cff00ccff[AishCore DIAG]|r " .. tostring(msg))
  end
end
ns.DPrint = DPrint
...
-- Slash command : /aish ou /aishcore
SLASH_AISHCORE1 = "/aish"
...
SlashCmdList["AISHCORE"] = function(msg)
  ...
  if msg == "diag" or msg == "diag on" or msg == "diag off" then
    ...
    print("|cff00ccff==== AishCore Diagnostic ====|r")
  ...
  elseif msg == "rh debug" then
    ...
    if rh and rh.ToggleDebug then
      ...
      print("|cff00b0ff[AishCore]|r RotationHelper module not found")
    end
  elseif msg == "rh drag" then
    ...
    if rh then
      ...
      print("|cff00b0ff[AishCore]|r Rotation Helper drag " .. (rh._dragEnabled and "ON" or "OFF"))
    end
  ...
  elseif msg == "ps debug" then
    ...
    if pb and pb.ToggleDebug then
      ...
      print("|cff00b0ff[AishCore]|r PriorityBar module not found")
    end
  ...
  elseif msg:find("^aura%s+") then
    ...
    if arg == "scan" then
      print("|cff00b0ff[AishCore]|r Scan buffs joueur :")
      for i = 1, 64 do
      ...
      if not sid then
        print("|cff00b0ff[AishCore]|r Usage: /aish aura <spellID> | /aish aura scan")
      else
        local aura = C_UnitAuras.GetPlayerAuraBySpellID(sid)
        if not aura then
          print("|cff00b0ff[AishCore]|r Aura " .. sid .. " : non trouvée via GetPlayerAuraBySpellID")
        else
          print("|cff00b0ff[AishCore]|r Aura " .. sid .. " trouvée :")
  ...
  elseif msg:find("^charges%s+") or msg == "charges" then
    ...
    local sid = tonumber(arg)
    if not sid then
      print("|cff00b0ff[AishCore]|r Usage : /aish charges <spellID>")
      return
    end
    local pb = ns.Modules.PriorityBar
  ...
  elseif msg == "ttb debug" then
    ...
    if ttb and ttb.Debug then
      ...
      print("|cff00b0ff[AishCore]|r TopTargetBar module introuvable")
    end
    -- Debug forcé de l'indicateur de zone de repos
    local xb = ns.Modules.XPBar
    if not xb then print("|cffff4444[AishCore]|r XPBar module introuvable"); return end
    ...
    print("|cff00b0ff[AishCore]|r REST DEBUG :")
  ...
  elseif msg == "testbar" then
    ...
    if ns._testBar then
      ...
      ns._testBar = nil
      print("|cff00ccff[AishCore]|r testbar fermé")
      return
    end
    ...
    print("|cff00ccff[AishCore]|r testbar ouvert — /aish testbar pour fermer")
  elseif msg == "arc force" then
    ...
    if rc then
      if rc.SetCenterArcValue then
        ...
        print("|cff00ccff[AishCore]|r arc force → arc à 70% pendant 5s")
      elseif rc.SetCenterArcEntry then
        print("|cffff4444[AishCore]|r SetCenterArcValue absent, teste SetCenterArcEntry...")
      else
        print("|cffff4444[AishCore]|r ResourceCircle sans SetCenterArcValue")
      end
    else
...
frame:RegisterUnitEvent("UNIT_POWER_FREQUENT", "player")  -- ressources continues (mana/energy/rage/focus...) : anim d'arc plus fluide
...
frame:SetScript("OnEvent", function(self, event, arg1, ...)
  if event == "ADDON_LOADED" and arg1 == addonName then
    ...
    if not ns.Profiles or not ns.Profiles.InitDB then
      ...
      if not ok then
        print("|cffff4444[AishCore]|r LoadDatabase a échoué : " .. tostring(err))
      else
  ...
  elseif event == "PLAYER_ENTERING_WORLD" then
    ...
    if ns.MinimapButton then
      ...
      if not ok then print("|cffff4444[AishCore]|r MinimapButton.Init : " .. tostring(err)) end
    else
    ...
    if not ok then print("|cffff4444[AishCore]|r CachePlayerSpec : " .. tostring(err)) end
    ...
    if ns.ScanAuraStacks then
      ...
      if not ok2 then print("|cffff4444[AishCore]|r ScanAuraStacks : " .. tostring(err2)) end
    else
